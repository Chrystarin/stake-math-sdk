...
- **Wheel**: 54 segments. `x1` 21, `x2` 13, `x5` 7, `x10` 4, `chest` 4, `piratePlinko` 2,
  `oceanVoyage` 2, `bonusWheel` 1 — Crazy Time's own split, so the Bonus Wheel is the one-off
  segment. Layout in `SEGMENT_LAYOUT`: one room every six segments, so exactly five numbers sit
  between any two rooms (chests every 12, Pirate Plinko and Ocean Voyage opposite pairs, the
  Bonus Wheel on its own); x10 never next to x10.
...
reached at >= 1 in 20,000,000 (`MAX_WIN_FLOOR`). The binding max-win case is Pirate Plinko's 400x slot
...
Because every spot returns 96.7%, every combination does too (linearity), so all 252 modes
...
game_config.py       GameConfig + 252 combination BetModes (is_feature) + 5 buy BetModes (is_buybonus)
